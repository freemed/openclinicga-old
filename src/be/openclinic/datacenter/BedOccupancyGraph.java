...
public class BedOccupancyGraph {
    ...
    */
    public static List getListValueGraph(int serverId, String serviceuid, Date dateBegin, String sLanguage, String userid) {
        Connection conn = null;
        PreparedStatement ps = null;
        ...
        try {
            TimeSeries series = new TimeSeries("data");
            conn = MedwanQuery.getInstance().getStatsConnection();
            ps = conn.prepareStatement("select * from DC_BEDOCCUPANCYVALUES where DC_BEDOCCUPANCYVALUE_SERVERID=? and DC_BEDOCCUPANCYVALUE_SERVICEUID=? and DC_BEDOCCUPANCYVALUE_DATE>? order by DC_DIAGNOSISVALUE_DATE");
            ps.setInt(1, serverId);
            ps.setString(2, serviceuid);
            ps.setTimestamp(3, new java.sql.Timestamp(dateBegin.getTime()));
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                java.util.Date dDate = new java.util.Date(rs.getTimestamp("DC_DIAGNOSISVALUE_YEAR").getTime());
