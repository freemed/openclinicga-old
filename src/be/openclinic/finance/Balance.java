    ...
    public static double getPatientBalance(String personid){
        ...
        try{
            String sSelect = "select sum(total) balance from" +
                    ...
                    " where" +
                    " a.oc_patientcredit_encounteruid="+ MedwanQuery.getInstance().convert("varchar(10)","b.oc_encounter_serverid")+MedwanQuery.getInstance().concatSign()+"'.'"+MedwanQuery.getInstance().concatSign()+ MedwanQuery.getInstance().convert("varchar(10)","b.oc_encounter_objectid")+" and" +
                    " b.oc_encounter_patientuid=?" +
                    " union" +
                    " select -sum(oc_debet_amount) total from oc_debets a,oc_encounters b" +
                    " where" +
                    " a.oc_debet_encounteruid="+ MedwanQuery.getInstance().convert("varchar(10)","b.oc_encounter_serverid")+MedwanQuery.getInstance().concatSign()+"'.'"+MedwanQuery.getInstance().concatSign()+ MedwanQuery.getInstance().convert("varchar(10)","b.oc_encounter_objectid")+" and" +
                    " b.oc_encounter_patientuid=?" +
                    ") a";
