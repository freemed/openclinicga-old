...
public class MealItem extends OC_Object {
    ...
    public void updateOrInsert(String useruid) {
        ...
        try {
            ...
            if (getUid().equals("-1")) {
                // insert new item
                sSelect = "INSERT INTO OC_MEAL_ITEM (OC_MEAL_ITEM_SERVERID, OC_MEAL_ITEM_OBJECTID," +
                        "  OC_MEAL_ITEM_NAME, OC_MEAL_ITEM_DESCRIPTION, OC_MEAL_ITEM_UNIT,OC_MEAL_ITEM_CREATETIME,OC_MEAL_ITEM_UPDATEUID)" +
                        " VALUES(?,?,?,?,?,?,?)";
                ps = oc_conn.prepareStatement(sSelect);
                // OBJECT variables
                ids = new String[]{MedwanQuery.getInstance().getConfigString("serverId"), MedwanQuery.getInstance().getOpenclinicCounter("OC_MEALS_ITEMS") + ""};
                ...
                sSelect = "UPDATE OC_MEAL_ITEM SET" +
                        "  OC_MEAL_ITEM_NAME=?,OC_MEAL_ITEM_UPDATETIME=?,OC_MEAL_ITEM_UPDATEUID=?, OC_MEAL_ITEM_DESCRIPTION=?, OC_MEAL_ITEM_UNIT=?" +
            ...
            setNutricientItems(useruid);//update nutricient items
        } catch (Exception e) {
    ...
    public void setNutricientItems(String useruid) {
        deleteAllNutricientItemsFromMealItem();//first delete all mealitems from meal
        if (this.nutricientItems != null && this.nutricientItems.size() > 0) {
            PreparedStatement ps = null;
            ResultSet rs = null;
            String sSelect;
            Iterator it = this.nutricientItems.iterator();
            while (it.hasNext()) {
                ...
                Connection oc_conn=MedwanQuery.getInstance().getOpenclinicConnection();
                try {
                    sSelect = "INSERT INTO OC_NUTRICIENT_COMPONENT (OC_NUTRICIENT_MEAL_ITEMUID, OC_NUTRICIENT_ITEMUID," +
    ...
    public void deleteAllNutricientItemsFromMealItem() {
        ...
        Connection oc_conn=MedwanQuery.getInstance().getOpenclinicConnection();
        try {
            String sSelect = "DELETE FROM OC_NUTRICIENT_COMPONENT" +
    ...
    public void delete() {
        deleteAllNutricientItemsFromMealItem();// first delete all nutricientItems
        PreparedStatement ps = null;
        ResultSet rs = null;
        Connection oc_conn=MedwanQuery.getInstance().getOpenclinicConnection();
        try {
            String sSelect = "DELETE FROM OC_MEAL_ITEM" +
                    " WHERE OC_MEAL_ITEM_SERVERID = ? AND OC_MEAL_ITEM_OBJECTID = ?";
            ps = oc_conn.prepareStatement(sSelect);
    ...
    public static MealItem get(MealItem item) {
        ...
        Connection oc_conn=MedwanQuery.getInstance().getOpenclinicConnection();
        try {
            String sSelect = "SELECT * FROM OC_MEAL_ITEM WHERE OC_MEAL_ITEM_SERVERID = ? AND OC_MEAL_ITEM_OBJECTID = ?";
            ps = oc_conn.prepareStatement(sSelect);
        ...
        return importNutricientItems(item);
    }
    public static MealItem importNutricientItems(MealItem item) {
        ...
        Connection oc_conn=MedwanQuery.getInstance().getOpenclinicConnection();
        try {
            String sSelect = "SELECT * FROM OC_NUTRICIENT_COMPONENT WHERE OC_NUTRICIENT_MEAL_ITEMUID = ? ";
            ps = oc_conn.prepareStatement(sSelect);
    ...
    public static List getList(MealItem findItem) {
        ...
        Connection oc_conn=MedwanQuery.getInstance().getOpenclinicConnection();
        try {
            String sSelect = "SELECT * FROM OC_MEAL_ITEM WHERE 1=1";
            if (findItem.name != null && findItem.name.trim().length() > 0) {
                sSelect += " AND OC_MEAL_ITEM_NAME LIKE '%" + findItem.name + "%'";
            }
            if (findItem.description != null && findItem.description.trim().length() > 0) {
                sSelect += " AND OC_MEAL_ITEM_DESCRIPTION LIKE '%" + findItem.description + "%'";
            }
            sSelect += " ORDER BY OC_MEAL_ITEM_NAME";
            ps = oc_conn.prepareStatement(sSelect);
            // execute
            rs = ps.executeQuery();
            while (rs.next()) {
