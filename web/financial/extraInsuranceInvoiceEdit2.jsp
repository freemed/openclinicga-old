...
function doSave() {
    ...
    if ((EditForm.EditDate.value.length > 0) && (EditForm.EditStatus.selectedIndex > -1 && EditForm.EditInsurarUID.value.length>0)) {
        ...
        var today = new Date();
        var url = '<c:url value="/financial/extraInsuranceInvoiceSave2.jsp"/>?ts=' + today;
...
function loadOpenInsurarInvoices() {
    var params = '';
    var today = new Date();
    var url = '<c:url value="/financial/extraInsurarInvoiceGetOpenInsurarInvoices2.jsp"/>?ts=' + today;
    new Ajax.Request(url, {
        method: "GET",
        parameters: params,
        onSuccess: function(resp) {
...
function changeInsurar() {
    ...
    var url = '<c:url value="/financial/extraInsurarInvoiceGetPrestations2.jsp"/>?ts=' + <%=getTs()%>;
    ...
    var url = '<c:url value="/financial/extraInsurarInvoiceGetCredits2.jsp"/>?ts='  + <%=getTs()%>;
