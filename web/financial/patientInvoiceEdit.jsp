                ...
                if(sPatientCreditUID.length() > 0){
                    ...
                    if(patientcredit!=null){
                        ...
                        sReturn.append("<tr class='list"+sClass+"'>")
                                .append("<td><input type='checkbox' name='cbPatientInvoice"+patientcredit.getUid()+"="+patientcredit.getAmount()+"' id='"+patientcredit.getType()+"."+patientcredit.getUid()+"' onclick='doBalance(this, false)'"+sChecked+"></td>")
                                .append("<td>"+ScreenHelper.getSQLDate(patientcredit.getDate())+"</td>")
                                .append("<td>"+getTran("credit.type",checkString(patientcredit.getType()),sWebLanguage)+"</td>")
                                .append("<td align='right'>"+patientcredit.getAmount()+" "+MedwanQuery.getInstance().getConfigParam("currency","€")+"</td>")
                               .append("</tr>");
                    }
                }
            }
...
<%
    ...
    if(sFindPatientInvoiceUID.length() > 0){
        ...
        if(patientInvoice!=null && patientInvoice.getDate()!=null){
        	...
        	out.print(getTran("web","invoice.does.not.exist",sWebLanguage)+": "+sFindPatientInvoiceUID);
        }
    }
    else{
	...
	<form name='FindForm' id="FindForm" method='POST'>
	    ...
	    <table class="menu" width="100%" cellspacing="0" cellpadding="0">
	        <tr>
	            <td width="<%=sTDAdminWidth%>" class="admin2"><%=getTran("web.finance","invoiceid",sWebLanguage)%></td>
	            <td class="admin2">
	                ...
	                <img src="<c:url value="/_img/icons/icon_search.gif"/>" class="link" alt="<%=getTranNoLink("Web","select",sWebLanguage)%>" onclick="searchPatientInvoice();">
	...
	<div id="divOpenPatientInvoices" class="searchResults" style="height:120px;width:100%"></div>
	
	<script>
	  function searchPatientInvoice(){
	    openPopup("/_common/search/searchPatientInvoice.jsp&FindInvoicePatient=<%=sPatientId%>&doFunction=doFind()&ReturnFieldInvoiceNr=FindPatientInvoiceUID&FindInvoicePatientId=<%=sPatientId%>&Action=search&header=false&PopupHeight=550&ts=<%=getTs()%>");
	  }
	...
	<form name='EditForm' id="EditForm" method='POST'>
	    <table class='list' border='0' width='100%' cellspacing='1' cellpadding="0">
	        <tr>
	            <%-- INVOICE NUMBER + INSURAR REFERENCE --%>
	            ...
	            <td class="admin2">
	                ...
	                <%
	                	...
	                	if(checkString(patientInvoice.getInvoiceUid()).length()==0 && MedwanQuery.getInstance().getConfigString("multiplePatientInvoiceSeries","").length()>0){
	                		...
	                		out.print("<input type='radio' class='text' name='invoiceseries' value='0'/>"+getTran("web","internal",sWebLanguage));
	                		
	                		for(int n=0; n<invoiceSeries.length; n++){
	                    		out.print("<input type='radio' class='text' name='invoiceseries' value='"+invoiceSeries[n]+"'/>"+invoiceSeries[n]);
	                		}
	                	}
        ...
        %>
	        ...
	        <tr>
	            ...
	            <td class='admin2'>
                    ...
                    &nbsp;<input type="button" class="button" name="update" value="<%=getTranNoLink("web","update",sWebLanguage)%>" onclick="loadDebets();"/>
	        ...
	        if(patientInvoice==null || patientInvoice.getStatus()==null || patientInvoice.getStatus().equalsIgnoreCase("open") || MedwanQuery.getInstance().getConfigInt("enableInvoiceVerification",0)==1){
		            ...
		            <% 	
		            	if(patientInvoice==null || patientInvoice.getStatus()==null || patientInvoice.getStatus().equalsIgnoreCase("open")){ 
			            	...
			            	<td class='admin2'>
			                    ...
			                    &nbsp;<input type="button" class="button" name="update2" value="<%=getTranNoLink("web","update",sWebLanguage)%>" onclick="loadDebets();"/>
							</td>
	                        <%
						}
						else{
  			...
  			if(patientInvoice!=null){
	           	...
	           	if(patientInvoice!=null && patientInvoice.getStatus()!=null && patientInvoice.getStatus().equals("open") && insurance!=null && insurance.getInsurar()!=null && insurance.getInsurar().getAllowedReductions()!=null && insurance.getInsurar().getAllowedReductions().length()>0){
					out.print("<tr><td class='admin'>"+getTran("web","acceptable.reductions",sWebLanguage)+"</td><td class='admin2' colspan='3'>");
	    ...
	    <tr>
	        ...
	        <td class='admin2' colspan='3'>
	            <div class="searchResults" style="height:120px;width:100%" id="patientInvoiceDebets" name="patientInvoiceDebets"></div>
	   
		        <div style="padding-top:3px;">
		            <input class="button" type="button" id="ButtonDebetSelectAll" name="ButtonDebetSelectAll" value="<%=getTranNoLink("web","selectall",sWebLanguage)%>" onclick="selectAll('cbDebet',true,'ButtonDebetSelectAll','ButtonDebetDeselectAll',true);">&nbsp;
		            <input class="button" type="button" id="ButtonDebetDeselectAll" name="ButtonDebetDeselectAll" value="<%=getTranNoLink("web","deselectall",sWebLanguage)%>" onclick="selectAll('cbDebet',false,'ButtonDebetDeselectAll','ButtonDebetSelectAll',true);">
		        </div>
	        </td>
	    </tr>
	    ...
	    <tr>
	        ...
	        <td class='admin2' colspan='3'>
	            <div class="searchResults" style="height:120px;width:50%">	            
		            <table id='creditsTable' width="100%" cellspacing="1" cellpadding="0">
		                <%-- header --%>
		                <tr class="gray">
		                    <td width="20"/>
		                    <td width="80"><%=getTran("web","date",sWebLanguage)%></td>
		                    <td width="250"><%=getTran("web","type",sWebLanguage)%></td>
		                    <td width="100" align="right"><%=getTran("web","amount",sWebLanguage)%>&nbsp;</td>
		                </tr>
		                ...
		                <%
		                    ...
		                    if(!(checkString(patientInvoice.getStatus()).equalsIgnoreCase("closed")||checkString(patientInvoice.getStatus()).equalsIgnoreCase("canceled"))){
		                        ...
		                        out.print("<tr><td colspan='4'>"+vUnassignedCredits.size()+" "+getTranNoLink("web","records.loaded",sWebLanguage)+"</td></tr>");
		                    }
		                %>
		            </table>
		        </div>
		        ...
		        <div style="padding-top:3px;">
	                <input class="button" type="button" id="ButtonPatientInvoiceSelectAll" name="ButtonPatientInvoiceSelectAll" value="<%=getTranNoLink("web","selectall",sWebLanguage)%>" onclick="selectAll('cbPatientInvoice',true,'ButtonPatientInvoiceSelectAll', 'ButtonPatientInvoiceDeselectAll',false);">&nbsp;
	                <input class="button" type="button" id="ButtonPatientInvoiceDeselectAll" name="ButtonPatientInvoiceDeselectAll" value="<%=getTranNoLink("web","deselectall",sWebLanguage)%>" onclick="selectAll('cbPatientInvoice',false,'ButtonPatientInvoiceDeselectAll', 'ButtonPatientInvoiceSelectAll',false);">
	            </div>
	        </td>
	    </tr>
...
<script>	
  function doValidate(invoiceuid){
    var url = '<c:url value="/financial/patientInvoiceValidate.jsp"/>?ts='+new Date();
  ...
  function doSign(invoiceuid){
    var url = '<c:url value="/financial/patientInvoiceSign.jsp"/>?ts='+new Date();
    new Ajax.Request(url,{
      method: "POST",
      postBody: 'EditInvoiceUID='+invoiceuid,
      onSuccess: function(resp){
  ...
  function doSave(){
    ...
    if((document.getElementById('EditDate').value.length>8) && (!document.getElementById('invoiceStatus').selectedIndex || document.getElementById('invoiceStatus').selectedIndex>-1)&&bInvoiceSeries){
      ...
      if(dateFormat=="dd/MM/yyyy"){
    	  invoiceDate = new Date(document.getElementById('EditDate').value.substring(6)+"/"+
    		                     document.getElementById('EditDate').value.substring(3,5)+"/"+
    		                     document.getElementById('EditDate').value.substring(0,2));
      }
      else{
    	  invoiceDate = new Date(document.getElementById('EditDate').value.substring(6)+"/"+
    		                     document.getElementById('EditDate').value.substring(0,2)+"/"+
      ...
      if(invoiceDate > new Date()){
        ...
        alertDialog("web.manage","dateinfuture");
      }
      <%
          ...
          if(!canCloseUnpaidInvoice){
              %>
                  ...
                  alertDialog("web.manage","cannotcloseunpaidinvoice");
                }
              <%
          }
      %>
	    ...
	    if((document.getElementById('EditBalance').value*1==0)&&(document.getElementById('invoiceStatus').value!="closed")&&(document.getElementById('invoiceStatus').value!="canceled")){
	      if(yesnoDialog("web.finance","closetheinvoice")){
  ...
  function doPrintPdf(invoiceUid){
    if(<%=activeUser.getAccessRight("financial.printopeninvoice.select")?"1":"0"%>==0 && ("<%=sClosed%>"!="closed")&&("<%=sClosed%>"!="canceled")){
      alertDialog("<%=getTranNoLink("web","closetheinvoicefirst",sWebLanguage)%>");
    }
    else{
  ...
  function doModifyInvoice(invoiceuid){
    var url= '<c:url value="/financial/recreateInvoice.jsp"/>?invoiceuid='+invoiceuid+'&ts='+new Date();
    document.getElementById('patientInvoiceDebets').innerHTML = "<img src='<%=sCONTEXTPATH%>/_img/themes/<%=sUserTheme%>/ajax-loader.gif'/><br/>Loading";
    new Ajax.Request(url,{
	  method: "GET",
      parameters: "",
      onSuccess: function(resp){
  ...
  function doPrintPatientReceipt(invoiceUid){
    var url= '<c:url value="/financial/printPatientReceiptOffline.jsp"/>?invoiceuid='+invoiceUid+'&ts='+new Date()+
  ...
  function loadOpenPatientInvoices(){
    ...
    document.getElementById('divOpenPatientInvoices').innerHTML = "<img src='<%=sCONTEXTPATH%>/_img/themes/<%=sUserTheme%>/ajax-loader.gif'/><br/>Loading";
    new Ajax.Request(url,{
	  method: "GET",
      parameters: "",
      onSuccess: function(resp){
