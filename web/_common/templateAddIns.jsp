...
<script>
 ...
 //*************************************************//
   ...
   //******************  ITEMS FIELDS AUTOCOMPLETION  *****************//

function ItemsAutocompleter(inputId, divId, page, params) {
    new Ajax.Autocompleter(inputId,
            divId,
            page,
    {parameters:params,tokens: '<%=MedwanQuery.getInstance().getConfigString("autocomplementTokens")%>',
        paramName:'itemValue'

    });
}
//****************** TEST ITEMS VALUES BY AJAX *********************//
...
function testItemValue(type, value)
{
    ...
    new Ajax.Request('_common/search/searchByAjax/autocompletionItemsValuesShow.jsp', {asynchronous:false,method: "POST",
        postBody: '&itemValue=' + value + "&item=" + type + "&testValue=1"});
...
function setPopup(itemType, itemValue) {
    ...
    //document.getElementById("ie5menu").style.display='';
    if (ie) {
        if (itemValue != null) {
...
function checkNavigation(evt) {
    if (window.myButton) {
        //lastevt = (evt) ? evt : ((window.event) ? window.event : "");
