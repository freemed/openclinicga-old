...
<%!
    ...
    private Planning setDates(Planning planning, HttpServletRequest request, SimpleDateFormat fullDateFormat) throws Exception {
       try{
            ...
            if (sPlanningDateDay.length() > 0) {
                if (checkString(sPlanningDateHour).length() == 0) {
                    planning.setPlannedDate(fullDateFormat.parse(sPlanningDateDay + " 00:00"));
                } else if (checkString(sPlanningDateMinutes).length() == 0) {
                    planning.setPlannedDate(fullDateFormat.parse(sPlanningDateDay + " " + sPlanningDateHour + ":00"));
                } else {
                    planning.setPlannedDate(fullDateFormat.parse(sPlanningDateDay + " " + sPlanningDateHour + ":" + sPlanningDateMinutes));
                }
            }
            Date enddate = planning.getPlannedDate();
            if (sPlanningDateEndDay.length() > 0) {
                if (checkString(sPlanningDateEndHour).length() == 0) {
                    enddate = (fullDateFormat.parse(sPlanningDateDay + " 00:00"));
                } else if (checkString(sPlanningDateEndMinutes).length() == 0) {
                   enddate = (fullDateFormat.parse(sPlanningDateDay + " " + sPlanningDateEndHour + ":00"));
                } else {
                    enddate = (fullDateFormat.parse(sPlanningDateDay + " " + sPlanningDateEndHour + ":" + sPlanningDateEndMinutes));
                }
            }
    ...
    if (sAction.equalsIgnoreCase("save")) {
        ...
        try {
            ...
            if(planning.updateDate(sEditPlanningUID)){
                out.write("<script>clientMsg.setValid('"+HTMLEntities.htmlentities(getTranNoLink("web.control","dataissaved",sWebLanguage))+"',null,1000)</script>");
            }
        }
        catch (Exception e) {
            out.write("<script>clientMsg.setError('"+HTMLEntities.htmlentities(getTranNoLink("web.control","dberror",sWebLanguage))+"');</script>");
...
<table class='list' border='0' width='630' cellspacing='1'>
    <tr>
        ...
        <td class="admin2">
            <%-- hour --%> <select id="appointmentDateHour" name="appointmentDateHour" class="text">
            <%for (int n = Integer.parseInt(sFrom); n <= Integer.parseInt(sUntil); n++) {
   ...
   <tr>
        ...
        <td class="admin2">
            <%-- hour --%> <select id="appointmentDateEndHour" name="appointmentDateEndHour" class="text">
            <%for (int n = Integer.parseInt(sFrom); n <= Integer.parseInt(sUntil); n++) {
                out.print("<option value='" + (n < 10 ? "0" + n : "" + n) + "' ");
    ...
    <tr>
        <td class='admin'><%=HTMLEntities.htmlentities(getTran("planning", "patient", sWebLanguage))%>
        </td>
        <td class='admin2'>
