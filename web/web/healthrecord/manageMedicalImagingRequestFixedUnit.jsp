        ...
        <tr>
            ...
            <td class='admin2'>
                <select id="examination" class="text" name="currentTransactionVO.items.<ItemVO[hashCode=<mxs:propertyAccessorI18N name="transaction.items" scope="page" compare="type=be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_MIR2_TYPE" property="itemId"/>]>.value">
                    <option value="0" <mxs:propertyAccessorI18N name="transaction.items" scope="page" compare="type=be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_MIR2_TYPE;value=0" property="value" outputString="selected"/>><%=getTran("Web","choose",sWebLanguage)%>
...
<script>
  ...
  <%-- SUBMIT FORM --%>
  function submitForm(){
    var typeSelect = document.all['currentTransactionVO.items.<ItemVO[hashCode=<mxs:propertyAccessorI18N name="transaction.items" scope="page" compare="type=be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_MIR2_TYPE" property="itemId"/>]>.value'];
  ...
  <%-- DO PRINT PDF --%>
  function doPrintPDF(){
    window.open('<c:url value="/healthrecord/loadPDF.jsp"/>?file=base/<%=sWebLanguage%>4CO.pdf&module=N4CO&modulepar1='+document.all['examination'].options[document.all['examination'].selectedIndex].text+'&modulepar2='+document.all['other'].value+'&modulepar3='+document.all['remark'].value+'&modulepar4='+document.all['reason'].value+'$'+document.getElementById('applying_physician').value+'&ts=<%=ScreenHelper.getTs()%>','Print','toolbar=yes, status=yes, scrollbars=yes, resizable=yes, width=700, height=500,menubar=yes');
  }
