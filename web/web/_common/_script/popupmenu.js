...
function hidemenuie5() {
  if (!bShowMenu){
//werkt niet wanneer er op het hoofdmenu geklikt wordt
...
// layout voor als men over een item komt met de muis
...
// layout voor als men over een item is geweest met de muis
function lowlightie5(evt) {
    if(evt.target){
        if (evt.target.className == "menuitems") {
            //item krijgt terug de normale kleur (zoals gedefinieerd in highlightie5) bij ""
...
/*        iLeft = oObject.offsetLeft;
        iRight = oObject.offsetLeft+oObject.offsetHeight;

        var sfSelect = document.getElementsByTagName("SELECT");
        for (var i=0; i<sfSelect.length; i++){
            rcts = sfSelect[i].getClientRects();
            if ((rcts[0].left > iLeft)&&(rcts[0].left < iRight)) {
