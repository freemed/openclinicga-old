...
%>
            ...
            <input type="text" name="barcode" size="8" class="text" style="{visibility: hidden;color: #FFFFFF;background: #EEEEEE; background-color: #EEEEEE;border-style: none;}" onKeyDown="if(enterEvent(event,13)){readBarcode2(this.value);}"/>
            <%
...
<img class="link" onclick="window.location = '<c:url value="/main.do" />?Page=meals/manageMeals.jsp&ts='+new Date().getTime()" alt="<%=getTranNoLink("web","meals",sWebLanguage)%>" title="<%=getTranNoLink("meals","meals",sWebLanguage)%>"  src="<c:url value='/_img/icon_meals.png'/>" />
