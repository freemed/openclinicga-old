...
<form id="transactionForm" name="transactionForm" method="POST" action='<c:url value="/healthrecord/updateTransaction.do"/>?ts=<%=getTs()%>' onclick="setSaveButton(event);" onkeyup="setSaveButton(event);">
    ...
    <table class="list" cellspacing="1" cellpadding="0" width="100%">
        ...
        <tr>
            <td class="admin"><%=getTran("Web","kine.manager",sWebLanguage)%></td>
            <td class="admin2">
                <select class='text' name="currentTransactionVO.items.<ItemVO[hashCode=<mxs:propertyAccessorI18N name="transaction.items" scope="page" compare="type=be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_CNAR_CONSULTATION_KINE" property="itemId"/>]>.value">
                    <option/>
                    <%
                        out.print(ScreenHelper.writeSelect("encounter.kine.manager",tran.getItemValue("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_CNAR_CONSULTATION_KINE"),sWebLanguage));
                    %>
                </select>
            </td>
        </tr>
