...
<table class="list" width="100%" cellspacing="1">
    <tr class="admin">
        <td align="center"><a href="javascript:openPopup('healthrecord/findDSM4.jsp&PopupWidth=700&PopupHeight=400&ts=<%=getTs()%>&patientuid=<%=activePatient.personid %>')"><%=getTran("openclinic.chuk","diagnostic.document",sWebLanguage)%> <%=getTran("Web.Occup","DSM4",sWebLanguage)%>/<%=getTran("Web.Occup","ICD-10",sWebLanguage)%></a></td>
    </tr>
    <tr>
        <td id='dsm4codes'>
	        <table width='100%'>
			         ...
			         while (items.hasNext()) {
			             ...
			             if (item.getType().indexOf("DSM4Code") == 0) {
			     			...
			     			%><tr id="DSM4Code<%=item.getItemId()%>"><td width="1%" nowrap>
			                         <img src="<c:url value='/_img/icon_delete.gif'/>" onclick="deleteDiagnosis('DSM4Code<%=item.getItemId()%>');"/>
...
<table class="list" width="100%" cellspacing="1">
    <tr class="admin">
        <td align="center"><%=getTran("openclinic.chuk","contact.diagnoses",sWebLanguage)%> <%=getTran("Web.Occup","DSM4",sWebLanguage)%>/<%=getTran("Web.Occup","ICD-10",sWebLanguage)%></td>
    </tr>
    <tr>
        <td>
	        <table width='100%'>
