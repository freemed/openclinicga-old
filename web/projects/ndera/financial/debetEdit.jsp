...
<form name='EditForm' id="EditForm" method='POST'>
    ...
    <table class='list' border='0' width='100%' cellspacing='1'>
		...
		<%
			...
			if(debet.getPatientInvoiceUid()!=null && debet.getPatientInvoiceUid().length()>0){
				PatientInvoice invoice = PatientInvoice.get(debet.getPatientInvoiceUid());
				invoiceClosed=invoice!=null && invoice.getStatus().equalsIgnoreCase("closed");
			}
			if(!invoiceClosed){
		%>
		        <tr>
		...
		%>
		        <tr>
		            <td class="admin"/>
		            <td class="admin2"><%=getTran("Web","caredeliverylinkedtoclosedinvoice",sWebLanguage)%></td>
		        </tr>
		<%
			}
        %>
...
<script>
  ...
  function doSave(){
      if(EditForm.EditDate.value.length>8){
          var debetDate = new Date(EditForm.EditDate.value.substring(6)+"/"+EditForm.EditDate.value.substring(3,5)+"/"+EditForm.EditDate.value.substring(0,2));
	      if(debetDate> new Date()){
	      ...
	      else if((EditForm.EditInsuranceUID.value.length>0)
	...
	function openQuicklist(){
	    openPopup("/financial/quicklist.jsp&ts=<%=getTs()%>&EditInsuranceUID=&PopupHeight=500&PopupWidth=800");
	}
