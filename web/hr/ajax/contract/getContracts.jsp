...
<%
	...
	//findObject.beginDate = ScreenHelper.stdDateFormat.parse(sBeginDate);
	//findObject.endDate = ScreenHelper.stdDateFormat.parse(sEndDate);
    ...
    if(contracts.size() > 0){
	    ...
	    for(int i=0; i<contracts.size(); i++){
	        ...
	        hSort.put(contract.beginDate.getTime()+"="+contract.getUid(),
	    	          ...
	    	          "<td class='hand' style='padding-left:5px'>"+checkString(contract.functionCode)+"</td>"+
...
<table width="100%" class="sortable" id="searchresults" cellspacing="1" style="border-bottom:none;">
    ...
    <tr class="admin" style="padding-left: 1px;">
        ...
        <td width="10%" nowrap><%=HTMLEntities.htmlentities(getTran("web.hr","duration",sWebLanguage))%></td>
        <td width="10%" nowrap><%=HTMLEntities.htmlentities(getTran("web.hr","functionCode",sWebLanguage))%></td>
        <td width="50%" nowrap><%=HTMLEntities.htmlentities(getTran("web.hr","functionTitle",sWebLanguage))%></td>
        <%-- <td width="30%" nowrap><%=HTMLEntities.htmlentities(getTran("web","ref1",sWebLanguage))%></td> --%>
    </tr>
