    ...
    <table class="list" border="0" width="100%" cellspacing="1">
        ...
        <tr>
            ...
            <td class="admin2">
                ...
                <%=writeDateField("careerEnd","EditForm",ScreenHelper.getSQLDate(career.end),sWebLanguage)%>            
            </td>                        
        </tr>
        ...
        <%
            ...
            if(checkString(career.contractUid).length() > 0){
                sContractId = career.contractUid.substring(1);                
            }
        %>   
        <tr>
            <td class="admin"><%=getTran("web.hr","contract",sWebLanguage)%>&nbsp;*&nbsp;</td>
            <td class="admin2">
                <input type="hidden" name="contract" id="contract" value="<%=checkString(career.contractUid)%>">
                <input type="text" class="text" name="contractId" id="contractId" <%--readonly--%> size="20" value="<%=sContractId%>">
                   
                <%-- buttons --%>
                <img src="<c:url value="/_img/icon_search.gif"/>" class="link" alt="<%=getTran("web","select",sWebLanguage)%>" onclick="searchContract('contract','contractId');">
                <img src="<c:url value="/_img/icon_delete.gif"/>" class="link" alt="<%=getTran("web","clear",sWebLanguage)%>" onclick="document.getElementById('contract').value='';document.getElementById('contractId').value='';">
            </td>
        </tr>
        ...
        <tr>
            ...
            <td class="admin2">
                <select class="text" id="grade" name="grade">
                    <option/>
                    <%=ScreenHelper.writeSelect("hr.career.grade",checkString(career.grade),sWebLanguage)%>
                </select>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td class="admin2">
                <select class="text" id="status" name="status"> 
                    <option/>
                    <%=ScreenHelper.writeSelect("hr.career.status",checkString(career.status),sWebLanguage)%>
                </select>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td class="admin2">
                <textarea class="text" name="comment" cols="80" rows="4" onKeyup="resizeTextarea(this,8);"><%=checkString(career.comment)%></textarea>
            </td>
        </tr>
        ...
        <tr>     
            ...
            <td class="admin2" colspan="2">
                ...
                <input class="button" type="button" name="buttonDelete" value="<%=getTranNoLink("web","delete",sWebLanguage)%>" onclick="deleteCareer();" style="visibility:hidden;">&nbsp;
                <input class="button" type="button" name="buttonNew" value="<%=getTranNoLink("web","new",sWebLanguage)%>" onclick="newCareer();" style="visibility:hidden;">&nbsp;
            </td>
        </tr>
    </table>
  ...
  function saveCareer(){
    ...
    if(document.getElementById("careerBegin").value.length > 8 &&
      ...
      if(document.getElementById("careerEnd").value.length > 0){
        ...
        var careerEnd = makeDate(document.getElementById("careerEnd").value);
      ...
      else if(document.getElementById("contractName").value.length==0) document.getElementById("contractName").focus(); // todo : 'contract' ipv 'contractName'
    ...
    new Ajax.Request(url,
      {
        ...
        parameters: "PatientId=<%=activePatient.personid%>",
                    //"&contractUid=<%=career.contractUid%>"+
                    //"&position=<%=career.position%>"+
                    //"&serviceUid=<%=career.serviceUid%>"+
                    //"&grade=<%=career.grade%>"+
