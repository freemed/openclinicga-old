...
<form name="EditForm" id="EditForm" method="POST">
    <input type="hidden" id="EditDisRecUid" name="EditDisRecUid" value="<%=checkString(disRec.getUid())%>">
    ...
    <table class="list" border="0" width="100%" cellspacing="1">
        ...
        <tr>
            ...
            <td class="admin2"> 
                <%=writeDateField("date","EditForm",ScreenHelper.getSQLDate(disRec.date),sWebLanguage)%>            
            </td>                        
        </tr>
        ...
        <tr>
            ...
            <td class="admin2">
                <input type="text" class="text" id="title" name="title" size="50" maxLength="255" value="<%=checkString(disRec.title)%>">
            </td>
        </tr>
        ...
        <tr>
            ...
            <td class="admin2">
                <textarea class="text" name="description" cols="80" rows="4" onKeyup="resizeTextarea(this,8);"><%=checkString(disRec.description)%></textarea>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td class="admin2">
                <select class="text" id="decision" name="decision">
                    <option/>
                    <%=ScreenHelper.writeSelect("hr.disrec.decision",checkString(disRec.decision),sWebLanguage)%>
                </select>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td class="admin2">
                <input type="text" class="text" id="duration" name="duration" size="3" maxLength="3" value="<%=(disRec.duration > 0?disRec.duration:"")%>" onKeyUp="isNumber(this);"> <%=getTran("web","days",sWebLanguage)%>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td class="admin2">
                <input type="text" class="text" id="decisionBy" name="decisionBy" size="50" maxLength="100" value="<%=checkString(disRec.decisionBy)%>">
            </td>
        </tr>
        ...
        <tr>
            ...
            <td class="admin2">
                <textarea class="text" name="followUp" cols="80" rows="4" onKeyup="limitChars(this,6000);resizeTextarea(this,8);"><%=checkString(disRec.followUp)%></textarea>
            </td>
        </tr>
    ...
    new Ajax.Request(url,
      {
        ...
        parameters: "PatientId=<%=activePatient.personid%>",
                    //"&date=<%=disRec.date%>"+
                    //"&title=<%=disRec.title%>"+
                    //"&description=<%=disRec.description%>"+
                    //"&decision=<%=disRec.decision%>"+
                    //"&duration=<%=disRec.duration%>"+
                    //"&decisionBy=<%=disRec.decisionBy%>"+
                    //"&followUp=<%=disRec.followUp%>",
        onSuccess: function(resp){
